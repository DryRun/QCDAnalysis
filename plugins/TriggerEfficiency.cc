...
#include "CMSDIJET/QCDAnalysis/plugins/TriggerEfficiency.h"
...
void TriggerEfficiency::endJob() 
{
	...
	for (std::map<TString, std::map<TString, unsigned int> >::iterator it_hlt = trigger_counts_raw_.begin(); it_hlt != trigger_counts_raw_.end(); ++it_hlt) {
		for (std::map<TString, unsigned int>::iterator it_l1 = (*it_hlt).second.begin(); it_l1 != (*it_hlt).second.end(); ++it_l1) {
			std::cout << "HLT " << (*it_hlt).first << " / L1 " << (*it_l1).first << " : PS = " << trigger_counts_prescaled_[(*it_hlt).first][(*it_l1).first] << " / " << trigger_counts_raw_[(*it_hlt).first][(*it_l1).first] << " = " << trigger_counts_prescaled_[(*it_hlt).first][(*it_l1).first] / trigger_counts_raw_[(*it_hlt).first][(*it_l1).first] << std::endl;
		}
	}
	mInf->Close();
}
		...
		for(unsigned i=0;i<NN;i++) {
			...
			for(unsigned itrig=0;itrig<mRefTrigger.size();itrig++) {
				...
				if (hltPass) {
					...
					// Prescale counters
					// Loop over the L1 seeds
					for (std::vector<std::pair<std::string, int> >::iterator it_l1 = mEvent->preL1(ihlt).begin(); it_l1 != mEvent->preL1(ihlt).end(); ++it_l1) {
						TString hlt_name = mRefTrigger[itrig];
						TString l1_name = (*it_l1).first;
						if (trigger_counts_raw_.find(hlt_name) == trigger_counts_raw_.end()) {
							trigger_counts_raw_[hlt_name][l1_name] = 0;
							trigger_counts_prescaled_[hlt_name][l1_name] = 0;
						} else if (trigger_counts_raw_[hlt_name].find(l1_name) == trigger_counts_raw_[hlt_name].end()) {
							trigger_counts_raw_[hlt_name][l1_name] = 0;
							trigger_counts_prescaled_[hlt_name][l1_name] = 0;
						}
						trigger_counts_raw_[hlt_name][l1_name] += 1;
						trigger_counts_prescaled_[hlt_name][l1_name] += (*it_l1).second * mEvent->preHLT(ihlt);
