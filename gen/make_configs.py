...
def MakeFastSimCfg(p_model, p_mass_point):
	...
	for stage in stages:
		...
		command = "cmsDriver.py " + GetSimulationCFF(p_model, p_mass_point).replace("$CMSSW_BASE/src/", "") + " --python_filename " + GetConfigPath(p_model, p_mass_point, stage, "FASTSIM") + " --fileout file:" + GetOutputTag(p_model, p_mass_point, stage, "FASTSIM") + ".root --step GEN,FASTSIM,HLT:7E33v2 --mc --eventcontent " + stage + " --datatier GEN-SIM-DIGI-RECO --pileup 2012_Startup_inTimeOnly --geometry DB --conditions auto:mc --beamspot Realistic8TeVCollision --no_exec -n 100"
		print command
		os.system(command)
