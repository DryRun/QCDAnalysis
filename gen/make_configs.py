...
def MakeFullSimCfgs(p_model, p_mass_point):
	...
	for stage in stages:
		...
		AddSubjobOption(GetConfigPath(p_model, p_mass_point, stage, "FULLSIM"), GetOutputTag(p_model, p_mass_point, stage, "FULLSIM"))
...
def MakeFastSimCfg(p_model, p_mass_point):
	command = "cmsDriver.py " + GetSimulationCFF(p_model, p_mass_point).replace("$CMSSW_BASE/src/", "") + " --python_filename " + GetConfigPath(p_model, p_mass_point, "RECOSIM", "FASTSIM") + " --fileout file:" + GetOutputTag(p_model, p_mass_point, "RECOSIM", "FASTSIM") + ".root --step GEN,FASTSIM,HLT:7E33v2 --mc --eventcontent RECOSIM --datatier GEN-SIM-DIGI-RECO --pileup 2012_Startup_inTimeOnly --geometry DB --conditions auto:mc --beamspot Realistic8TeVCollision --no_exec -n 1000"
	print command
	os.system(command)
	AddSubjobOption(GetConfigPath(p_model, p_mass_point, "RECOSIM", "FASTSIM"), GetOutputTag(p_model, p_mass_point, "RECOSIM", "FASTSIM"))
...
def AddSubjobOption(p_cfg, p_output_tag):
	...
	for line in old:
		...
		if "import FWCore.ParameterSet.Config as cms" in line:
		...
		elif "# Path and EndPath definitions" in line:
			# Random number seed
			new.write("\n")
			...
			new.write(")\n\n")
			new.write(line)
		elif p_output_tag in line and "fileName = cms.untracked.string":
