	...
	for wp in ["CSVT", "CSVM", "CSVL"]:
		...
		for which in [0, 1]:
			...
			for ht_slice in ht_slices:
				...
				this_numerator_histogram_3D = f.Get("BHistograms/h_pfjet_jet" + str(which) + "_pt_eta_" + wp)
				this_denominator_histogram_3D = f.Get("BHistograms/h_pfjet_jet" + str(which) + "_pt_eta")
		...
		for which in [0, 1, "both"]:
			c = TCanvas("c_single_btag_eff_" + sr + "_" + wp + "_jet" + str(which), "c_single_btag_eff_" + sr + "_" + wp + "_jet" + str(which), 800, 600)
